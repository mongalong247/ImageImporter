...
# Import the theme library for automatic dark/light mode
...
class ImageImporter(QMainWindow): # Changed from QWidget to QMainWindow
    ...
    def __init__(self):
        ...
        # --- Main Layout Setup for QMainWindow ---
        ...
        # New: Load all saved settings on startup
        self.load_settings()

    def build_import_form(self):
        # (This function's internal widgets are the same)
    ...
    def select_source_files(self):
        # (Functionality is the same, but adds validation reset)
        ...
        if files:
            ...
            self.source_folder = os.path.dirname(files[0]) # Remember this directory
            self.source_path_label.setText(f"{len(files)} file(s) selected")
            self.source_path_label.setToolTip("\n".join(files))
            self.source_path_label.setStyleSheet(NORMAL_STYLE) # Reset style on success
    ...
    def select_destination(self):
        ...
        if folder:
            ...
            self.dest_path_label.setStyleSheet(NORMAL_STYLE) # Reset style on success
    ...
    def select_backup(self):
        ...
        if folder:
            ...
            # No error style for optional backup
    ...
    def start_import(self):
        # New: Use the improved validation function.
        if not self._validate_paths():
            return
        
        # (The rest of the confirmation and worker setup is the same)
        try:
...
if __name__ == "__main__":
    ...
    # New: Set up automatic dark/light mode theme switching.
    # This requires the user to 'pip install pyqtdarktheme'.
